            ...
            <div className="flex items-center space-x-3 sm:space-x-8">
                ...
                <div className="hidden md:flex items-center space-x-6">
                    <div className="flex items-center space-x-2 bg-indigo-50 px-4 py-2 rounded-2xl border border-indigo-100">
                        ...
                        <div>
                            <p className="text-[9px] font-black text-indigo-400 uppercase leading-none">Integrity</p>
                            <p className="text-sm font-black text-indigo-900 tracking-tight tabular-nums">{userDoc?.points || 0}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="relative">
                    ...
                    >
                        <Bell size={22} />
                        <span className="absolute top-1 sm:top-2 right-1 sm:right-2 w-2.5 h-2.5 bg-indigo-500 rounded-full border-2 border-white"></span>
                    </button>
                    ...
                    {showNotifications && (
                        <div className="absolute right-0 mt-4 w-[calc(100vw-2rem)] sm:w-96 bg-white rounded-[2rem] shadow-2xl border border-slate-100 p-6 animate-in slide-in-from-top-4 duration-300">
                            <div className="flex justify-between items-center mb-6">
                                <h4 className="text-lg font-black text-slate-800 tracking-tighter">System Intel</h4>
                                <button className="text-[10px] font-black uppercase text-indigo-600">Clear</button>
                            </div>
                            <div className="py-10 text-center">
                                <p className="text-xs font-black text-slate-400 uppercase tracking-widest">No active system alerts</p>
                            </div>
                        </div>
                    )}
                </div>
                ...
                {/* Profile Card */}
                <div className="flex items-center space-x-3 bg-slate-50 p-1.5 sm:p-2 rounded-2xl border border-slate-100 hover:bg-white hover:border-indigo-200 transition-all cursor-pointer group">
                    ...
                    <div className="hidden sm:block xl:block leading-none pr-2">
                        <p className="text-[10px] sm:text-xs font-black text-slate-800 tracking-tight">{firstName}</p>
                        <p className="text-[8px] sm:text-[9px] font-black text-slate-400 uppercase mt-1 tracking-widest">{userDoc?.role || 'Student'}</p>
                    </div>
